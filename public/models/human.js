export class Human {
    constructor(positionX, positionY) {
        ...
        this.name = 'testName';
        let maleHeight = 50;
        this.humanHeight = this.isMale ? maleHeight : maleHeight / 1.2;
        this.humanWidth = this.humanHeight / 1.8;
    }
    draw(ctx) {
    ...
    //a way to draw using svg
    // private drawSketch(ctx: CanvasRenderingContext2D) {
    //     ctx.save();
    //     ctx.translate(-34, -93);
    //     ctx.beginPath();
    ...
    drawHead(ctx) {
        let radius = this.humanWidth / 5;
        let lineWidth = this.humanWidth / 10;
        ...
        ctx.beginPath();
        ctx.setLineDash([]);
        ctx.arc(0, 0 - this.humanHeight / 2 + lineWidth + radius / 2, radius, 0, Math.PI * 2, false);
    ...
    drawBody(ctx) {
        ...
        ctx.beginPath();
        if (this.isMale) {
            ctx.moveTo(0 - this.humanWidth / 2, 0 - this.humanHeight / 10);
            ctx.lineTo(0, this.humanHeight / 2);
            ctx.lineTo(0 + this.humanWidth / 2, 0 - this.humanHeight / 10);
            ctx.closePath();
        }
        else {
            ctx.moveTo(0, 0 - this.humanHeight / 10);
            ctx.lineTo(0 - this.humanWidth / 2, this.humanHeight / 2);
            ctx.lineTo(0 + this.humanWidth / 2, this.humanHeight / 2);
            ctx.closePath();
        }
        //ctx.lineCap = 'round';
